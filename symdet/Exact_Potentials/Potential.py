...
class Potential:
    ...
    args:
        k_range (list) -- Bin width with which to sample the data, e.g. [-5, 5]
        bin_operation (list) -- operations to apply to k values to get bins from multiplication to addition,
                                e.g. [1/5, 1e-3] leads to bins of [k/5 - 1e-3, k/5 + 1e-3]
        n_class_members (int) -- Amount of class representatives to have.
        coordinates (tf tensor) -- Tensor of coordinates to use in the function evaluation
    """
    ...
    def __init__(self, k_range, bin_operation, n_class_members=None, coordinates=None):
        ...
        if n_class_members is None:
            self.n_class_members = 1000
        else:
            self.n_class_members = n_class_members
        if coordinates is None:
            self.coordinate_tensor = tf.random.uniform([self.n_class_members * 1000, 2], minval=0.0, maxval=1.6)
        else:
    ...
    def _cluster_data(self, predictions, training_data):
